  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.canvas, justifyContent: 'center', padding: GUTTER, gap: 20 }}>
      <View style={{ backgroundColor: COLORS.surface, borderRadius: RADIUS.large, padding: 24, gap: 14, borderCurve: 'continuous' }}>
        {error ? null : <ActivityIndicator color={COLORS.primary} size="large" accessibilityLabel="Preparing FocusFlow workspace" />}
        <Text selectable style={{ color: COLORS.ink, fontSize: 24, lineHeight: 30, fontWeight: '900', textAlign: 'center' }}>{error ? 'Your workspace is still safe.' : 'Preparing FocusFlow'}</Text>
