...
export function AddTaskFab() {
  return <Pressable accessibilityRole="button" accessibilityLabel="Add task" onPress={() => router.push('/add-task')} style={({ pressed }) => [{ position: 'absolute', right: 20, bottom: 96, width: 58, height: 58, borderRadius: 20, backgroundColor: COLORS.primary, alignItems: 'center', justifyContent: 'center', boxShadow: DEEP_SHADOW }, pressed && { opacity: 0.8, transform: [{ scale: 0.96 }] }]}><Glyph name="plus" size={24} color={COLORS.white} /></Pressable>;
}
...
export function UndoToast({ task, onUndo, onDismiss }: { task: Task | null; onUndo: () => void; onDismiss: () => void }) {
  ...
  if (!task) return null;
  return <View accessibilityLiveRegion="polite" style={{ position: 'absolute', left: 20, right: 20, bottom: 98, minHeight: 54, borderRadius: RADIUS.medium, backgroundColor: COLORS.contrast, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, boxShadow: DEEP_SHADOW }}><Glyph name="check" size={15} color={COLORS.primary} /><Text selectable numberOfLines={1} style={{ flex: 1, color: COLORS.contrastText, fontSize: 13, fontWeight: '700' }}>{task.title} completed</Text><Pressable accessibilityRole="button" accessibilityLabel={`Undo completion of ${task.title}`} onPress={onUndo} style={{ minWidth: 48, minHeight: 44, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.primary, fontSize: 13, fontWeight: '900' }}>Undo</Text></Pressable></View>;
}
