...
export function TaskSection({ title, subtitle, tasks, onComplete, emptyTitle, emptyBody }: { title: string; subtitle?: string; tasks: TaskV3[]; onComplete: (task: TaskV3) => void; emptyTitle: string; emptyBody: string }) {
...
export function AddTaskFab() {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel="Add task" onPress={() => router.push('/add-task')} style={({ pressed }) => [{ position: 'absolute', right: 20, bottom: Math.max(18, bottomInset + 10), width: 58, height: 58, borderRadius: 20, backgroundColor: COLORS.primary, alignItems: 'center', justifyContent: 'center', boxShadow: DEEP_SHADOW }, pressed && { opacity: 0.8, transform: [{ scale: 0.96 }] }]}><Glyph name="plus" size={24} color={COLORS.white} /></Pressable>;
}
