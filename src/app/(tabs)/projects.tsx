...
export default function ProjectsScreen() {
  ...
  const quietProjects = projects.filter((project) => project.status !== 'active');
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.canvas }}>
      <ScrollView contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: GUTTER, paddingTop: 20, paddingBottom: 158, gap: 24 }}>
        ...
        <SurfaceCard style={{ padding: 18, gap: 14, backgroundColor: COLORS.primarySoft }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}><View style={{ width: 28, height: 28, borderRadius: 10, backgroundColor: COLORS.contrast, alignItems: 'center', justifyContent: 'center' }}><Glyph name="trend" size={15} color={COLORS.primary} /></View><Text style={{ color: COLORS.ink, fontSize: 14, fontWeight: '900' }}>A little momentum goes a long way.</Text></View><Text style={{ color: COLORS.muted, fontSize: 12, lineHeight: 18, fontWeight: '600' }}>{completedTasks} tasks are wrapped across your workspace. Keep the next action smaller than the ambition.</Text></SurfaceCard>
